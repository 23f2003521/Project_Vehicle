...
def handle_overstays():
    ...
    with app.app_context():
        ...
        for res in overstayed_reservations:
            ...
            hours_overstayed = int((now - (leaving_time + grace_period)).total_seconds() // 3600)
            penalty = hours_overstayed * penalty_per_hour
            ...
            html_template = """
            <h3>Dear {{ username }},</h3>
            <p><strong>Notice:</strong> You have exceeded your parking time by more than 24 hours at <strong>{{ lot_name }}</strong>.</p>
